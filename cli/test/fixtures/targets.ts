...
export function baseTargets(withDeps = false) {
  ...
  const programACommand = targets.resolvePathToObject(path.join(cwd, `qcmdsrc`, `programA.cmd`));
  ...
  const unusedCmd = targets.resolvePathToObject(path.join(cwd, `qcmdsrc`, `unused.cmd`));
  ...
  // Program object
  const programA = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `programA.pgm.rpgle`));
  ...
  const programB = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `programB.pgm.sqlrpgle`));
  ...
  // Program object, imports TOLOWER
  const programC = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `programC.pgm.sqlrpgle`));
  ...
  const moduleA = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `moduleA.rpgle`));
  ...
  const moduleB = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `moduleB.sqlrpgle`));
  ...
  const srvpgmAModule = targets.resolvePathToObject(path.join(cwd, `qsrvsrc`, `srvpgmA.bnd`));
  ...
  // FILEA.FILE
  const fileA = targets.resolvePathToObject(path.join(cwd, `qddssrc`, `fileA.sql`));
  ...
  // FILEB.FILE
  const fileB = targets.resolvePathToObject(path.join(cwd, `qddssrc`, `fileB.pf`));
  ...
  const ORDENTSRV = targets.resolvePathToObject(path.join(cwd, `qbndsrc`, `ordentsrv.binder`));
  ...
  const ORDENTMOD = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `ordentmod.rpgle`));
  ...
  const UNUSEDSRV = targets.resolvePathToObject(path.join(cwd, `qbndsrc`, `unusedsrv.binder`));
...
export function multiModuleObjects() {
  ...
  const myWebApp = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `mywebapp.pgm.rpgle`));
  ...
  const handlerAMod = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `handlerA.rpgle`));
  ...
  const handlerBMod = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `handlerB.rpgle`));
  ...
  const jwtHandlerMod = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `jwtHandler.rpgle`));
  jwtHandlerMod.exports = [`JWT_MIDDLEWARE`];

  // Another module that is part of the JWTHANDLER.SRVPGM object
  const validateMod = targets.resolvePathToObject(path.join(cwd, `qrpglesrc`, `validate.rpgle`));
  validateMod.exports = [`API_VALIDATE`];

  // Service program for JWTHANDLER, used by MYWEBAPP
  const jwtHandlerSrv = targets.resolvePathToObject(path.join(cwd, `qsrvsrc`, `utils.binder`));
