...
import { setupCompanySystem, setupFixture } from './fixtures/projects';
import { scanGlob } from '../src/extensions';
import { writeFileSync } from 'fs';
import { getDefaultCompiles } from '../src/builders/environment';

const cwd = setupFixture(`company_system_no_pgm_ext`)
...
describe.skipIf(files.length === 0)(`company_system tests`, () => {
  ...
  test(`Check mypgm`, async () => {
    ...
    expect(myPgm.relativePath).toBe(path.join(`qrpglesrc`, `mypgm.rpgle`));
    expect(myPgm.deps.length).toBe(0);
  });

  test(`Check employees`, async () => {
    const myPgm = targets.getTarget({systemName: `EMPLOYEES`, type: `PGM`});
    expect(myPgm.relativePath).toBe(path.join(`qrpglesrc`, `employees.sqlrpgle`));
  ...
  test(`Check depts`, async () => {
    ...
    expect(myPgm.relativePath).toBe(path.join(`qrpglesrc`, `depts.sqlrpgle`));
    ...
    expect(empPgm.relativePath).toBe(path.join(`qrpglesrc`, `employees.sqlrpgle`));
