...
export class rpgleTargetParser {
  private parser: Parser;
  private includeFileCache: { [path: string]: string } = {};

  constructor(private targets: Targets) {
    this.parser = this.setupParser(targets);
  }

  public async rpgleTargetCallback(targets: Targets, localPath: string, content: string, options: FileOptions) {
    const cache = await this.parser.getDocs(
      localPath,
      content,
      {
        ignoreCache: true,
    ...
    if (cache) {
      const ileObject = await this.targets.resolvePathToObject(localPath, options.text);
      ...
      if (cache.includes && cache.includes.length > 0) {
        ...
        cache.includes.forEach((include: IncludeStatement) => {
          ...
          if (includeDetail.ext.toLowerCase() !== `.rpgleinc`) {
            ...
            if (this.targets.suggestions.renames) {
              const renameLogPath = this.targets.getRelative(include.toPath);

              // We need to make sure the .rpgleinc rename is most important
              if (this.targets.logger.exists(renameLogPath, `rename`)) {
                this.targets.logger.flush(renameLogPath);
              }

              this.targets.logger.fileLog(renameLogPath, {
              ...
              this.targets.logger.fileLog(this.targets.getRelative(include.toPath), {
          ...
          const theIncludePath = asPosix(this.targets.getRelative(include.toPath));

          ileObject.headers.push(theIncludePath);

          if (this.targets.suggestions.includes) {
            this.targets.logger.fileLog(ileObject.relativePath, {
      ...
      cache.procedures
        ...
        .forEach((ref: RpgLookup) => {
          ...
          const resolvedObject = this.targets.searchForObject({ systemName: ref.lookup, type: `PGM` });
          if (resolvedObject) {
      ...
      for (const scope of scopes) {
        ...
        scope.structs
          ...
          .forEach((ref: RpgLookup) => {
            const resolvedObject = this.targets.searchForObject({ systemName: ref.lookup, type: `FILE` });
        ...
        scope.structs
          ...
          .forEach((ref: RpgLookup) => {
            ...
            const resolvedObject = this.targets.searchForObject({ systemName: ref.lookup, type: `DTAARA` });
            if (resolvedObject) target.deps.push(resolvedObject)
            else {
              this.targets.logger.fileLog(ileObject.relativePath, {
                message: `No object found for reference '${ref.lookup}'`,
      ...
      const resolvedObject = this.targets.searchForObject({ systemName: ileObject.systemName, type: `CMD` });
      if (resolvedObject) this.targets.createOrAppend(resolvedObject, target);

      if (target.deps.length > 0)
        infoOut(`Depends on: ${target.deps.map(d => `${d.systemName}.${d.type}`).join(` `)}`);

      this.targets.addNewTarget(target);
    }
  }

  setupParser(targets: Targets): Parser {
    ...
    parser.setIncludeFileFetch(async (baseFile: string, includeFile: string) => {
      ...
      if (file) {
        if (this.includeFileCache[file]) {
          return {
            found: true,
            uri: file,
            content: this.includeFileCache[file]
          }

        } else {
          const content = await targets.rfs.readFile(file);
          this.includeFileCache[file] = content;
